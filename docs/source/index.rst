...
Our multi-agent system attempts to generate creative monophonic songs with structure resembling that of compositions devised by human composers.


Short summary
=============

Each agent has its own Markov chain of notes, which has been initially created from real songs. On each iteration step every agent generates a short theme using their Markov chain. The agents evaluate all the created themes and the best one is chosen to be the base of a song. TODO: Filipe can surely explain how the song is created here.
